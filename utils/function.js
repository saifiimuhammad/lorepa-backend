...
module.exports = {
  ...
  sendOtp: async (phoneNumber, otp) => {
    try {
      const message = await client.messages.create({
        body: `Your OTP for hop on account verification is: ${otp}`,
  ...
  aISuggestion: async (note) => {
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
    const result = await model.generateContent([
      ` Give suggestion on this note : ${note}. make sure the response is in one line`,
    ]);
    return result.response.text();
  },
  aISuggestionOnRap: async (note) => {
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
    const result = await model.generateContent([
      ` Give suggestion on this rap line : ${note}. make sure the response is in one line and only give suggestion about the line`,
    ]);
    return result.response.text();
  },
  aIPerformanceAnalysis: async (note) => {
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
    const result = await model.generateContent([
      `Analyze this rap  : ${note}. and give perforamce analysis in the form of object only return object in response no additional text 
            {
                smoothness:80,
                creativity:10,
                versality10
